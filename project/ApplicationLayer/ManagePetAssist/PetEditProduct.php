...
$data = $pet->pet_view($sp_id,$pet_idd);
...
<body bgcolor="#ccd9ff">
  ...
  <table id="inside_part" width="70%" height="70%" align="center">
    <tr> <hr>
      ...
      ?>
      <input type="hidden" name="cur_pet_id" value="<?=$row['pet_id']?>">
    <tr>
      <td>Product Name: </td>
      <td><input type="text" name="name" size="30" value="<?php echo $row['pet_name']?>" required></td>
    ...
    <tr>
      <td>Product Price: </td>
      <td>RM <input type="number" step="0.01" name="price" value="<?php echo $row['pet_price']?>" required> </td>    
      <td colspan="2" rowspan="3" align="center"><img src='<?php echo $image_src?>' id="image" width="160" height="160"/></td>
    </tr>
    <tr>
      <td>Quantity: </td>
      <td><input type="number" name="quantity" value="<?php echo $row['pet_quantity']?>" required></td>
    </tr>
    <tr>
      <td>Variation: </td>
      <td><select name="pet_variation">
          <option value="" disabled selected value>-Select one pet variation-</option>
          <option value="Food" <?php if ($row['pet_variation'] == "Food" ) echo 'selected' ; ?> >Pet Food</option>
          <option value="Toy" <?php if ($row['pet_variation'] == "Toy" ) echo 'selected' ; ?> >Pet Toy</option>
      </select> </td>
    </tr>
    <tr>
      <td>Product Details: </td>
      <td><textarea rows="5" cols="40" name="detail" required><?php echo $row['pet_detail']?></textarea></td>
  ...
  <?php } ?>
    <tr>
      <td colspan="2"><button type="button" name="back" onclick="window.location.href='PetProduct.php'"> Back </button></td>
