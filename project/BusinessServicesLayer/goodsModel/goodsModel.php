...
class goodsModel{
    ...
    function goods_editProduct(){
        if($this->coverpath != ""){
            $sql = "update goods set gd_name=:gd_name,gd_detail=:gd_detail,gd_color=:gd_color,gd_size=:gd_size,gd_variation=:gd_variation,gd_coverpath=:gd_coverpath,gd_price=:gd_price,gd_quantity=:gd_quantity where gd_id=:gd_id and sp_id=:sp_id";
            $args = [':gd_name'=>$this->name, ':gd_detail'=>$this->detail, ':gd_color'=>$this->color, ':gd_size'=>$this->size,  ':gd_variation'=>$this->variation, ':gd_coverpath'=>$this->coverpath,':gd_price'=>$this->price,':gd_quantity'=>$this->quantity,':gd_id'=>$this->id, ':sp_id'=>$this->sp_id];
            $stmt = $this->run($sql, $args);
            $count = $stmt->rowCount();
            return $count;
        }
        else{
            $sql = "update goods set gd_name=:gd_name,gd_detail=:gd_detail,gd_color=:gd_color,gd_size=:gd_size,gd_variation=:gd_variation,gd_price=:gd_price,gd_quantity=:gd_quantity where gd_id=:gd_id and sp_id=:sp_id";
            $args = [':gd_name'=>$this->name, ':gd_detail'=>$this->detail, ':gd_color'=>$this->color, ':gd_size'=>$this->size,':gd_variation'=>$this->variation, ':gd_price'=>$this->price,':gd_quantity'=>$this->quantity,':gd_id'=>$this->id, ':sp_id'=>$this->sp_id];
            $stmt = $this->run($sql, $args);
            $count = $stmt->rowCount();
            return $count; 
        }
